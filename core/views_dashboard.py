...
def dashboard(request):
    ...
    # Action Items
    actions = {
        'rentals_start': Rental.objects.filter(rental_date=today, status='RESERVED').select_related('customer'),
        'rentals_return': Rental.objects.filter(return_date=today, status='PICKED_UP').select_related('customer'),
        'jobs_due': SeamstressJob.objects.filter(due_date=today).exclude(status='COMPLETED').select_related('customer'),
    ...
    context = {
        ...
        'active_rentals': Rental.objects.filter(status__in=['RESERVED', 'PICKED_UP']).select_related('customer').order_by('rental_date')[:5],
        'pending_jobs': SeamstressJob.objects.filter(status__in=['PENDING', 'IN_PROGRESS']).select_related('customer').order_by('due_date')[:5],
